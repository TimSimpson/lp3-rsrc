...
struct CentralDirectoryHeader {
    ...
    std::string_view get_extra_field() const {
        ...
        return std::string_view(name.data() + name.size(), this->extra_field_length);
    }

    std::string_view get_file_comment() const {
        const auto ef = this->get_extra_field();
        return std::string_view(ef.data() + ef.size(), this->file_comment_length);
    }

    std::ptrdiff_t real_size() const {
        return (sizeof(*this)) + (this->file_name_length + this->extra_field_length + this->file_comment_length);
    }
};
...
template <>
struct fmt::formatter<lp3::rsrc::FileHeader> {
  constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

  template <typename FormatContext>
  auto format(const lp3::rsrc::FileHeader& file_header, FormatContext& ctx) {
    format_to(ctx.out(), "{{ signature={0}{1}{2}{3}, ", file_header.signature[0], file_header.signature[1], file_header.signature[2], file_header.signature[3]);
    format_to(ctx.out(), "version={0}, ", file_header.version);
    format_to(ctx.out(), "flags={0}, ", file_header.flags);
    format_to(ctx.out(), "compression_method={0}, ", file_header.compression_method);
    format_to(ctx.out(), "dos_date_time={0}, ", file_header.dos_date_time);
    format_to(ctx.out(), "crc_of_file={0}, ", file_header.crc_of_file);
    format_to(ctx.out(), "uncompressed_file_size={0}, ", file_header.uncompressed_file_size);
    format_to(ctx.out(), "file_name_length={0}, ", file_header.file_name_length);
    format_to(ctx.out(), "extra_field_length={0} }}", file_header.extra_field_length);
    return ctx.out();
  }
};

template <>
struct fmt::formatter<lp3::rsrc::CentralDirectoryHeader> {
  constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

  template <typename FormatContext>
  auto format(const lp3::rsrc::CentralDirectoryHeader& s, FormatContext& ctx) {
    format_to(ctx.out(), "{{ signature={0}{1}{2}{3}, ", s.signature[0], s.signature[1], s.signature[2], s.signature[3]);
    format_to(ctx.out(), "version_made_by={}, ", s.version_made_by);
    format_to(ctx.out(), "host_os={}, ", s.host_os);
    format_to(ctx.out(), "minimum_version_needed_to_extract={}, ", s.minimum_version_needed_to_extract);
    ...
    format_to(ctx.out(), "dos_date_time_of_file={}, ", s.dos_date_time_of_file);
    format_to(ctx.out(), "crc_of_file={}, ", s.crc_of_file);
    format_to(ctx.out(), "compressed_file_size={}, ", s.compressed_file_size);
    format_to(ctx.out(), "uncompressed_file_size={}, ", s.uncompressed_file_size);
    ...
    format_to(ctx.out(), "external_file_attributes={}, ", s.external_file_attributes);
    format_to(ctx.out(), "relative_offset_of_local_header_from_start_of_first_disk={}", s.relative_offset_of_local_header_from_start_of_first_disk);
    format_to(ctx.out(), "}}");
    return ctx.out();
}
};
...
template <>
struct fmt::formatter<lp3::rsrc::EndOfDirectoryStructure> {
  constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

  template <typename FormatContext>
  auto format(const lp3::rsrc::EndOfDirectoryStructure& s, FormatContext& ctx) {
    format_to(ctx.out(), "{{ signature={0}{1}{2}{3}, ", s.signature[0], s.signature[1], s.signature[2], s.signature[3]);
    format_to(ctx.out(), "number_of_this_disk={}, ", s.number_of_this_disk);
    format_to(ctx.out(), "number_of_disk_with_start_of_central={}, ", s.number_of_disk_with_start_of_central);
    format_to(ctx.out(), "total_number_of_files={}, ", s.total_number_of_files);
    format_to(ctx.out(), "total_number_of_entries_in_central_dir={}, ", s.total_number_of_entries_in_central_dir);
    format_to(ctx.out(), "size_of_central_directory={}, ", s.size_of_central_directory);
    format_to(ctx.out(), "offset_of_start_of_central_directory={}, ", s.offset_of_start_of_central_directory);
    format_to(ctx.out(), "archive_comment_length={}", s.archive_comment_length);
    format_to(ctx.out(), "}}");
    return ctx.out();
  }
};
