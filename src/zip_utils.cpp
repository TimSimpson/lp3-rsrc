...
std::optional<LoadedDirectoryInfo> load_directory_info(lp3::sdl::RWops &file) {
    ...
    // info, then a end structure that has info on how many directories and
    // stuff there are.
    ...
    if (std::string_view(info.end_info.signature, 4)
        != std::string_view(end_of_directory_signature, 4)) {
    ...
    file.seek(-1
              * (sizeof(EndOfDirectoryStructure)
                 + info.end_info.size_of_central_directory));

    // reserver space in the buffer
    // TODO: this is wrong. Only reserve
    // `info.end_info.size_of_central_directory`
    info.buffer.resize(info.end_info.size_of_central_directory
                       + (info.end_info.total_number_of_entries_in_central_dir
                          * sizeof(CentralDirectoryHeader)));
